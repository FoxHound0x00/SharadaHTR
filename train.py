...
class Train(object):
                ...
                if batch_n > (len_train - 5):
                        ...
                        with torch.no_grad():
                            ...
                            for j in range(0, len(decoded)):
                                pred_label = decoded[j]
                                actual = yb.cpu().numpy()[0 + sum(lens[:j]): sum(lens[:j]) + lens[j]]
                                train_leven += leven.distance(''.join(pred_label.astype(str)), ''.join(actual.astype(str)))
            ...
            with torch.no_grad():
                ...
                for xb, yb, lens in self.valid_dl:
                    ...
                    for j in range(0, len(decoded)):
                        pred_label = decoded[j]
                        actual = yb.cpu().numpy()[0 + sum(lens[:j]): sum(lens[:j]) + lens[j]]
                        leven_dist += leven.distance(''.join(pred_label.astype(str)), ''.join(actual.astype(str)))
                        pred_len, actual_len = len(pred_label), len(actual)
                        mismatch = sum(pred_label[:min(pred_len, actual_len)] != actual[:min(pred_len, actual_len)]) + abs(len(pred_label) - len(actual))
                        cer += mismatch
                        if mismatch > 0:
                            wer += 1
    ...
    def predict(self, img_path=None, transforms=None, show_img=False, dev='cpu'):
        ...
        if show_img:
            ...
            plt.show()
        if transforms:
            img = transforms({'image':img, 'label':''})['image']
